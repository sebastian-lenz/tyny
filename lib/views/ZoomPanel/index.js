...
export class ZoomPanel extends View {
    ...
    getCoverViewProps() {
        return this.getCenteredViewProps(Math.max(this.height / this.getNativeHeight(), this.width / this.getNativeWidth()));
    }
    getFitToViewProps() {
        const { fitPadding } = this;
        return this.getCenteredViewProps(Math.min(1, (this.height - fitPadding * 2) / this.getNativeHeight(), (this.width - fitPadding * 2) / this.getNativeWidth()));
    }
