...
export default abstract class Timeline<TValue = any> {
  ...
  constructor(options: TimelineOptions<TValue>) {
    ...
    if (!accessor) {
      throw new Error(
        `Could not create an accessor for the property '${property}' on '${
          context
        }'.`
      );
    }
    ...
    if (!valueType) {
      throw new Error(
        `Could not resolve the value type of the property '${property}' on '${
          context
        }'.`
      );
    }
